...
const ITEM_TYPES: ItemTypeConfig[] = [
    {
        id: 'hourly',
        label: 'Por horas',
        description: 'Recurso × horas · asigna equipo en el ítem',
        Icon: Clock,
    ...
    },
    {
        id: 'recurring',
        label: 'Mensual',
        description: 'Retainer / fee · asigna equipo en el ítem',
        Icon: Repeat,
        ...
        <div className={cn('grid grid-cols-2 gap-3 sm:grid-cols-4', className)}>
            {ITEM_TYPES.map(({ id, label, description, Icon, tintClass, inkClass }) => (
                <button
                    ...
                    onClick={() => onAdd(id)}
                    className={cn(
                        'group flex flex-col items-start gap-3 rounded-xl border border-dashed p-4 text-left transition-all',
                        id === 'hourly' || id === 'recurring'
                            ? 'border-amber-200/90 bg-gradient-to-br from-amber-50/40 via-white to-white hover:border-amber-400 hover:bg-amber-50/60'
                            : 'border-gray-200 bg-white hover:border-primary hover:bg-primary-soft'
                    )}
                >
                    ...
                    <div>
                        ...
                        {(id === 'hourly' || id === 'recurring') && (
                            <p className="mt-2 text-[10px] font-bold uppercase tracking-[0.35px] text-amber-800">
                                Luego: asignar recurso al ítem
                            </p>
                        )}
                    </div>
                </button>
