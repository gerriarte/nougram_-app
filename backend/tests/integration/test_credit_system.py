...
class TestCreditAccountCreation:
    ...
    async def test_get_or_create_credit_account_creates_new(
        self, db_session: AsyncSession, org_free: Organization
    ):
    ...
    async def test_get_or_create_credit_account_returns_existing(
        self, db_session: AsyncSession, org_free: Organization
    ):
    ...
    async def test_credit_account_credits_per_month_by_plan(self, db_session: AsyncSession):
        ...
        account_starter = await CreditService.get_or_create_credit_account(
            org_starter.id, db_session
        )
        ...
        account_enterprise = await CreditService.get_or_create_credit_account(
            org_enterprise.id, db_session
        )
...
class TestSubscriptionCreditGrants:
    ...
    async def test_grant_subscription_credits_grants_correct_amount(
        self, db_session: AsyncSession, org_starter: Organization
    ):
    ...
    async def test_grant_subscription_credits_resets_monthly_counter(
        self, db_session: AsyncSession, org_free: Organization
    ):
        ...
        # Grant again (simulating monthly reset; force bypasses webhook dedup window)
        await CreditService.grant_subscription_credits(org_free.id, db_session, force=True)
    ...
    async def test_grant_subscription_credits_creates_transaction(
        self, db_session: AsyncSession, org_free: Organization
    ):
...
class TestCreditConsumption:
    ...
    async def test_consume_credits_decreases_available(
        self, db_session: AsyncSession, org_free: Organization
    ):
    ...
    async def test_consume_credits_creates_transaction(
        self, db_session: AsyncSession, org_free: Organization, product_manager_user: User
    ):
        ...
        consumption_txns = [
            ...
            if t.transaction_type == CreditService.TRANSACTION_TYPE_CONSUMPTION
        ]
    ...
    async def test_consume_credits_insufficient_raises_exception(
        self, db_session: AsyncSession, org_free: Organization
    ):
    ...
    async def test_unlimited_credits_do_not_consume(
        self, db_session: AsyncSession, org_enterprise: Organization
    ):
        """Test that organizations with unlimited credits do not consume credits"""
...
class TestManualCreditGrants:
    ...
    async def test_grant_manual_credits_increases_available(
        self, db_session: AsyncSession, org_free: Organization, super_admin_user: User
    ):
    ...
    async def test_grant_manual_credits_creates_transaction(
        self, db_session: AsyncSession, org_free: Organization, super_admin_user: User
    ):
        ...
        manual_txns = [
            t
            for t in transactions
            if t.transaction_type == CreditService.TRANSACTION_TYPE_MANUAL_ADJUSTMENT
        ]
...
class TestCreditRefunds:
    ...
    async def test_refund_credits_increases_available(
        self, db_session: AsyncSession, org_free: Organization
    ):
        ...
        await CreditService.grant_subscription_credits(org_free.id, db_session)
        await CreditService.validate_and_consume_credits(
            organization_id=org_free.id, amount=5, user_id=None, reason="Test", db=db_session
        )
    ...
    async def test_refund_credits_creates_transaction(
        self, db_session: AsyncSession, org_free: Organization
    ):
        ...
        refund_txns = [
            t for t in transactions if t.transaction_type == CreditService.TRANSACTION_TYPE_REFUND
        ]
...
class TestCreditBalance:
    ...
    async def test_get_credit_balance_returns_correct_info(
        self, db_session: AsyncSession, org_starter: Organization
    ):
        ...
        # Consume some credits
        await CreditService.validate_and_consume_credits(
            organization_id=org_starter.id, amount=30, user_id=None, reason="Test", db=db_session
        )
...
class TestCreditTransactionsHistory:
    ...
    async def test_transaction_history_includes_all_transactions(
        self, db_session: AsyncSession, org_free: Organization, super_admin_user: User
    ):
        ...
        # Consume credits
        await CreditService.validate_and_consume_credits(
            organization_id=org_free.id, amount=3, user_id=None, reason="Consumption", db=db_session
        )
        ...
        # Refund credits
        await CreditService.refund_credits(
            organization_id=org_free.id, amount=2, reason="Refund", db=db_session
        )
