...
async def org(db_session):
    o = Organization(
        name="T", slug=f"org-{uuid.uuid4().hex[:8]}",
...
async def _member(db, org_id, salary=5000, billable=40, currency="USD", non_billable=0.0):
    m = TeamMember(
        name="m", role="Dev", salary_monthly_brute=salary, currency=currency,
        billable_hours_per_week=billable, non_billable_hours_percentage=non_billable,
...
async def _won(db, org_id, service_id, items, status="Won", set_accepted=True, allocs=None):
    project = Project(name="P", client_name="C", status=status, organization_id=org_id, currency="USD")
    db.add(project)
    await db.flush()
...
class TestOperatingCost:
    async def test_fixed_costs(self, db_session, org):
        db_session.add(CostFixed(name="rent", amount_monthly=1000, currency="USD", category="Overhead", organization_id=org.id))
        await db_session.commit()
    ...
    async def test_equipment_amortization(self, db_session, org):
        db_session.add(EquipmentAmortization(
            name="laptop", category="Hardware", purchase_price=1200, purchase_date=date(2026, 1, 1),
            currency="USD", useful_life_months=12, salvage_value=0,
            depreciation_method="straight_line", is_active=True, organization_id=org.id,
        ))
        await db_session.commit()
        res = await calculate_monthly_operating_cost(db_session, "USD", org.id)
    ...
    async def test_social_charges_applied(self, db_session, org):
        org.settings = {"social_charges_config": {"enable_social_charges": True, "total_percentage": 30}}
        db_session.add(org)
        await db_session.commit()
...
class TestCommittedRevenue:
    async def test_recurring_mrr(self, db_session, org):
        ...
        await _won(db_session, org.id, svc.id, [
            {"pricing_type": "recurring", "recurring_price": 500, "quantity": 1, "billing_frequency": "monthly", "client_price": 500},
        ])
    ...
    async def test_annual_normalized(self, db_session, org):
        svc = await _service(db_session, org.id)
        await _won(db_session, org.id, svc.id, [
            {"pricing_type": "recurring", "recurring_price": 1200, "quantity": 1, "billing_frequency": "annual", "client_price": 1200},
        ])
        res = await calculate_committed_monthly_revenue(db_session, org.id, "USD")
    ...
    async def test_one_time_prorated(self, db_session, org):
        svc = await _service(db_session, org.id)
        await _won(db_session, org.id, svc.id, [{"pricing_type": "fixed", "client_price": 1200, "quantity": 1}])
        res = await calculate_committed_monthly_revenue(db_session, org.id, "USD", one_time_proration_months=12)
    ...
    async def test_ignores_non_won(self, db_session, org):
        ...
        await _won(db_session, org.id, svc.id, [
            {"pricing_type": "recurring", "recurring_price": 500, "quantity": 1, "billing_frequency": "monthly", "client_price": 500},
        ], status="Sent", set_accepted=False)
        res = await calculate_committed_monthly_revenue(db_session, org.id, "USD")
...
class TestBreakEven:
    async def test_deficit(self, db_session, org):
        db_session.add(CostFixed(name="rent", amount_monthly=5000, currency="USD", category="Overhead", organization_id=org.id))
        await db_session.commit()
    ...
    async def test_surplus(self, db_session, org):
        ...
        svc = await _service(db_session, org.id)
        await _won(db_session, org.id, svc.id, [
            {"pricing_type": "recurring", "recurring_price": 3000, "quantity": 1, "billing_frequency": "monthly", "client_price": 3000},
        ])
        res = await calculate_break_even(db_session, org.id, "USD")
...
class TestResourceUtilization:
    async def test_multiple_projects(self, db_session, org):
        ...
        svc = await _service(db_session, org.id)
        await _won(db_session, org.id, svc.id, [{"pricing_type": "hourly"}], allocs=[(0, {"team_member_id": m.id, "hours": 50})])
        await _won(db_session, org.id, svc.id, [{"pricing_type": "hourly"}], allocs=[(0, {"team_member_id": m.id, "hours": 50})])
    ...
    async def test_over_allocation(self, db_session, org):
        m = await _member(db_session, org.id)
        svc = await _service(db_session, org.id)
        await _won(db_session, org.id, svc.id, [{"pricing_type": "hourly"}], allocs=[(0, {"team_member_id": m.id, "hours": 200})])
        res = await calculate_resource_utilization(db_session, org.id)
    ...
    async def test_excludes_non_won(self, db_session, org):
        m = await _member(db_session, org.id)
        svc = await _service(db_session, org.id)
        await _won(db_session, org.id, svc.id, [{"pricing_type": "hourly"}],
                   status="Sent", set_accepted=False, allocs=[(0, {"team_member_id": m.id, "hours": 50})])
        res = await calculate_resource_utilization(db_session, org.id)
    ...
    async def test_date_window(self, db_session, org):
        m = await _member(db_session, org.id)
        svc = await _service(db_session, org.id)
        await _won(db_session, org.id, svc.id, [{"pricing_type": "hourly"}], allocs=[(0, {
            "team_member_id": m.id, "hours": 50,
