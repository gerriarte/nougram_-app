...
class TeamGroup(Base):
    ...
    __table_args__ = (UniqueConstraint("organization_id", "name", name="uq_team_groups_org_name"),)
...
class TeamCell(Base):
    ...
    organization_id = Column(Integer, ForeignKey("organizations.id"), nullable=False, index=True)
    group_id = Column(
        Integer, ForeignKey("team_groups.id", ondelete="RESTRICT"), nullable=False, index=True
    )
    name = Column(String(120), nullable=False)
    description = Column(String(500), nullable=True)
    is_active = Column(Boolean, nullable=False, server_default="true", default=True)
...
class TeamCellVersion(Base):
    ...
    organization_id = Column(Integer, ForeignKey("organizations.id"), nullable=False, index=True)
    cell_id = Column(
        Integer, ForeignKey("team_cells.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    members = relationship(
        "TeamCellMemberVersion", back_populates="cell_version", cascade="all, delete-orphan"
    )
...
class TeamCellMemberVersion(Base):
    ...
    cell_version_id = Column(
        Integer, ForeignKey("team_cell_versions.id", ondelete="CASCADE"), nullable=False, index=True
    )
    team_member_id = Column(
        Integer, ForeignKey("team_members.id", ondelete="RESTRICT"), nullable=False, index=True
    )
    weight = Column(
        Numeric(precision=10, scale=4), nullable=False, server_default="1.0", default=1.0
    )
