...
async def handle_checkout_session_completed(event: dict, db: AsyncSession):
    ...
    RepositoryFactory.create_subscription_repository(db)
...
async def handle_subscription_deleted(event: dict, db: AsyncSession):
    ...
    await org_repo.update_subscription(subscription.organization_id, status="cancelled")
...
async def handle_invoice_payment_failed(event: dict, db: AsyncSession):
    ...
    if subscription:
        ...
        # Update organization subscription status
        org_repo = RepositoryFactory.create_organization_repository(db)
        await org_repo.update_subscription(subscription.organization_id, status="past_due")
...
async def sync_subscription_from_stripe(
    stripe_subscription: dict, organization_id: int, db: AsyncSession
):
    """
    ...
    if not subscription:
        ...
        subscription = Subscription(
            ...
            trial_start=datetime.fromtimestamp(stripe_subscription["trial_start"], tz=UTC)
            if stripe_subscription.get("trial_start")
            else None,
            trial_end=datetime.fromtimestamp(stripe_subscription["trial_end"], tz=UTC)
        ...
        subscription.stripe_price_id = (
            stripe_subscription["items"]["data"][0]["price"]["id"]
            if stripe_subscription["items"]["data"]
            else subscription.stripe_price_id
        )
        subscription.status = stripe_subscription["status"]
        subscription.current_period_start = (
            datetime.fromtimestamp(stripe_subscription["current_period_start"], tz=UTC)
            if stripe_subscription.get("current_period_start")
            else None
        )
        subscription.current_period_end = (
            datetime.fromtimestamp(stripe_subscription["current_period_end"], tz=UTC)
            if stripe_subscription.get("current_period_end")
            else None
        )
    ...
    await org_repo.update_subscription(
        organization_id, plan=plan, status=stripe_subscription["status"]
    )
    ...
    try:
        ...
        logger.info(
            f"Granted subscription credits to organization {organization_id} for plan {plan}"
        )
    except Exception as e:
...
def _extract_plan_from_metadata(stripe_subscription: dict) -> str | None:
