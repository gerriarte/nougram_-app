...
@router.get("/templates", summary="Get onboarding inventory templates")
...
async def get_benchmarks(
    ...
    country: str | None = Query(
        None, description="Country code (defaults to organization country)"
    ),
    currency: str | None = Query(
        None, description="Currency code (defaults to organization currency)"
    ),
    tenant: TenantContext = Depends(get_tenant_context),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
...
@router.post(
    "/draft", response_model=OnboardingDraftResponse, summary="Save onboarding draft in backend"
)
...
@router.get(
    "/draft", response_model=OnboardingDraftResponse, summary="Get onboarding draft from backend"
)
...
async def import_preview_excel(
    file: UploadFile = File(
        ..., description="Excel file (.xlsx) with Organization, Team, Expenses, Inventory sheets"
    ),
    tenant: TenantContext = Depends(get_tenant_context),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
...
@router.get("/import-template/excel", summary="Download official onboarding Excel import template")
