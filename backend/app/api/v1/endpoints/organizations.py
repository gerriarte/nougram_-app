...
def require_super_admin_or_org_admin(
    current_user: User, organization_id: int | None = None
) -> bool:
...
@router.get(
    "/{organization_id}", response_model=OrganizationResponse, summary="Get organization by ID"
)
...
async def create_organization(
    ...
    **Note:** For public registration, use `POST /organizations/register` instead.
    ...
    try:
        ...
        logger.info(
            f"Granted initial subscription credits to organization {org.id} for plan {org.subscription_plan}"
        )
    except Exception as e:
        # Log error but don't fail organization creation
        logger.error(
            f"Error granting initial subscription credits to organization {org.id}: {e}",
            exc_info=True,
        )
...
@router.post(
    ...
    response_model=dict,
    status_code=status.HTTP_201_CREATED,
    summary="Register new organization (Public)",
)
...
async def list_organization_users(
    ...
    is_org_admin = current_user.organization_id == organization_id and getattr(
        current_user, "role", None
    ) in ["owner", "org_admin", "admin_financiero"]
...
@router.post(
    "/{organization_id}/invite", response_model=OrganizationInviteResponse, deprecated=True
)
async def invite_user_to_organization(
    ...
    **Permissions:**
    ...
    if not org:
        return OrganizationInviteResponse(
            success=False, message="Organization not found", invitation_token=None
        )

    # Check if user already exists and is in organization
    existing_user = await db.execute(select(User).where(User.email == invite_data.email))
    ...
    existing_invitation = await invitation_repo.get_by_email_and_org(
        invite_data.email, organization_id
    )
...
@router.get(
    ...
    response_model=OrganizationUsageStatsResponse,
    ...
    **Example Response:**
    ...
    service_count_result = await db.execute(
        select(func.count(Service.id)).where(
            Service.organization_id == organization_id, Service.deleted_at.is_(None)
        )
    )
    ...
    team_count_result = await db.execute(
        select(func.count(TeamMember.id)).where(
            TeamMember.organization_id == organization_id, TeamMember.is_active
        )
    )
...
async def add_user_to_organization(
    ...
    **Permissions:**
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="You don't have permission to add users"
        )
    ...
    if requested_role in SUPPORT_ROLES:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid role: {requested_role}"
        )
    ...
    if user_data.user_id:
        # Add existing user
        existing_user = await db.execute(select(User).where(User.id == user_data.user_id))
        user = existing_user.scalar_one_or_none()
...
async def update_user_role_in_organization(
    ...
    **Permissions:**
    ...
    """
    from app.core.permissions import get_user_role, validate_super_admin_email
    from app.core.roles import SUPPORT_ROLES, TENANT_ROLES

    current_role = get_user_role(current_user)
    is_super_admin = current_role == "super_admin"
    is_owner = current_role == "owner" and current_user.organization_id == organization_id
    ...
    # Check if trying to assign tenant role
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid role: {new_role}"
        )
    ...
    logger.info(
        f"User {user.id} role updated to {role_data.role} in organization {organization_id} by user {current_user.id}"
    )
...
async def remove_user_from_organization(
    ...
    if user.role == "org_admin":
        admin_count_result = await db.execute(
            select(func.count(User.id)).where(
                User.organization_id == organization_id, User.role == "org_admin"
            )
        )
    ...
    logger.info(
        f"User {user_id} removed from organization {organization_id} by user {current_user.id}"
    )
...
async def update_subscription_plan(
    ...
    current_user: User = Depends(
        get_current_user
    ),  # Permission check inside due to multi-tenant logic
    db: AsyncSession = Depends(get_db),
):
    """
    ...
    **Permissions:**
    ...
    org = await org_repo.update_subscription(
        organization_id, plan=subscription_data.plan, status=subscription_data.status
    )

    user_count = await org_repo.get_user_count(organization_id)

    logger.info(
        f"Subscription updated for organization {organization_id} to {subscription_data.plan} by user {current_user.id}"
    )
    ...
    if subscription_data.plan:
        try:
            ...
            logger.info(
                f"Granted subscription credits to organization {organization_id} for updated plan {subscription_data.plan}"
            )
        except Exception as e:
            # Log error but don't fail subscription update
            logger.error(
                f"Error granting subscription credits to organization {organization_id}: {e}",
                exc_info=True,
            )
...
@router.post(
    "/{organization_id}/onboarding-config",
...
async def save_onboarding_config(
    ...
    **Returns:**
    ...
    logger.info(
        f"Onboarding config saved for organization {organization_id} by user {current_user.id}"
    )
