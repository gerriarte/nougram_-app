...
async def get_dashboard_kpi_summary(
    ...
    start_date: date | None = Query(None, description="Custom range start (YYYY-MM-DD), local TZ"),
    end_date: date | None = Query(None, description="Custom range end (YYYY-MM-DD), local TZ"),
    timezone_name: str = Query(
        "America/Bogota", alias="timezone", description="IANA timezone for range boundaries"
    ),
...
async def get_dashboard_data(
    ...
    currency: str | None = Query(None, description="Filter by currency (USD, COP, ARS, EUR)"),
    ...
    **Permissions:**
    ...
    try:
        ...
        if start_date_obj:
            filters.append(
                Project.created_at >= datetime.combine(start_date_obj, datetime.min.time())
            )
        if end_date_obj:
            filters.append(
                Project.created_at <= datetime.combine(end_date_obj, datetime.max.time())
            )
        if currency:
        ...
        revenue_query = select(func.sum(Quote.total_client_price)).join(
        ...
        # Average margin
        margin_query = select(func.avg(Quote.margin_percentage)).join(
            Project, Quote.project_id == Project.id
        )
        if filters:
        ...
        projects_by_status = {row[0]: row[1] for row in projects_by_status_result.all()}
        ...
        client_query = (
            ...
            .join(Quote, Quote.project_id == Project.id, isouter=True)
        ...
        projects_by_client = [
            {"client_name": row[0], "project_count": row[1], "total_revenue": float(row[2] or 0)}
        ...
        revenue_by_service = {row[0]: float(row[1] or 0) for row in revenue_by_service_result.all()}

        # Total internal costs
        costs_query = select(func.sum(Quote.total_internal_cost)).join(
            Project, Quote.project_id == Project.id
        )
        if filters:
        ...
        team_hours_result = await db.execute(
            select(func.sum(TeamMember.billable_hours_per_week * 4.33)).where(
                TeamMember.is_active, TeamMember.organization_id == tenant.organization_id
            )
        )
        ...
        utilization_rate = (
            (total_used_hours / total_available_hours * 100) if total_available_hours > 0 else 0.0
        )
        ...
        for i in range(11, -1, -1):  # Last 12 months
            ...
            month_revenue_query = select(func.sum(Quote.total_client_price)).join(
                Project, Quote.project_id == Project.id
            )
        ...
        if compare_previous and start_date_obj and end_date_obj:
            ...
            prev_revenue_query = select(func.sum(Quote.total_client_price)).join(
                Project, Quote.project_id == Project.id
            )
