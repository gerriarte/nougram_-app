...
from app.core.permission_middleware import require_create_services
from app.core.permissions import PermissionError, can_edit
...
@router.delete(
    "/{service_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete service (soft delete)"
)
async def delete_service(
    ...
    **Permissions:**
    ...
    usage_result = await db.execute(
        select(func.count(QuoteItem.id)).where(QuoteItem.service_id == service_id)
    )
...
async def list_deleted_services(
    ...
    query = (
        select(Service)
        .where(Service.deleted_at.isnot(None), Service.organization_id == tenant.organization_id)
        .options(selectinload(Service.deleted_by))
    ...
    return ServiceListResponse(items=items, total=len(services))
...
async def permanently_delete_service(
    ...
    result = await db.execute(
        select(Service).where(Service.id == service_id, Service.deleted_at.isnot(None))
    )
