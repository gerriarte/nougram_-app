...
from app.core.permissions import ensure_role_string, require_super_admin
from app.core.security import get_current_user, get_password_hash
from app.models.user import User
from app.repositories.user_repository import UserRepository
from app.schemas.auth import UserCreate, UserListResponse, UserResponse, UserRoleUpdate
...
async def list_users(
    ...
    try:
        ...
        logger.warning(
            "Error checking super admin permission", error=str(e), user_id=current_user.id
        )
    ...
    try:
        ...
        logger.info(
            "Successfully loaded users using raw SQL",
    ...
    for user_data in users_data:
        ...
        try:
            ...
            logger.error(
                "Error creating UserResponse", user_id=user_data.get("id", "unknown"), error=str(e)
            )
...
async def create_user(
    ...
    # Create new user
    logger.info(
        "Creating user", email=normalized_email, role=user_data.role, user_id=current_user.id
    )
    new_user = User(
...
async def update_user_role(
    ...
    logger.info(
        "Updating user role", user_id=user_id, new_role=role_data.role, updated_by=current_user.id
    )
