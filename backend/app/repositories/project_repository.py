...
class ProjectRepository(BaseRepository[Project]):
    ...
    async def get_by_id_with_quotes(self, id: int, include_deleted: bool = False) -> Project | None:
        """
        ...
        query = (
            ...
            .options(
                ...
                .selectinload(QuoteItem.service),
                selectinload(Project.quotes)
                .selectinload(Quote.items)
                .selectinload(QuoteItem.allocations),
                selectinload(Project.quotes)
                .selectinload(Quote.items)
                .selectinload(QuoteItem.cell_assignment),
    ...
    async def get_all_with_quotes(
        self, include_deleted: bool = False, status_filter: str | None = None
    ) -> list[Project]:
        """
        ...
        query = select(Project).options(
            selectinload(Project.quotes).selectinload(Quote.items), selectinload(Project.taxes)
        )
    ...
    async def get_quote_by_id(self, quote_id: int) -> Quote | None:
        """
    ...
    async def get_latest_quote(self, project_id: int) -> Quote | None:
        """
    ...
    async def get_quote_with_relationships(self, quote_id: int, project_id: int) -> Quote | None:
        """
        ...
        if tax_ids:
            await self.db.execute(
                insert(project_taxes).values(
                    [{"project_id": project_id, "tax_id": tax_id} for tax_id in tax_ids]
                )
            )
            await self.db.flush()
