...
1. **frontend-ci.yml** - Pipeline único del frontend (`nougram_front/`)
   - ESLint, `tsc --noEmit`, `next build`
   - `npm run test --if-present` si existe script `test`

### Documentación / manual

1. **ci.yml** - Solo `workflow_dispatch` (mensaje informativo en el log)
